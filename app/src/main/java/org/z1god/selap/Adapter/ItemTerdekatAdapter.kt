...
class ItemTerdekatAdapter(val itemsLapangan : ArrayList<ItemLapanganModel>) : RecyclerView.Adapter<ItemTerdekatAdapter.MyViewHolder>() {
    ...
    inner class MyViewHolder(val view : View) : RecyclerView.ViewHolder(view) {
        ...
        val image = view.findViewById<ImageView>(R.id.iv_terdekat)
        val rating = view.findViewById<RatingBar>(R.id.rating)

        init {
            view.setOnClickListener {
                onItemClick?.invoke(itemsLapangan[adapterPosition])
            }
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_terdekat, parent,false)
        return MyViewHolder(view)
    }

    override fun getItemCount(): Int {
        return itemsLapangan.size
    }

    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        val item = itemsLapangan.get(position)
